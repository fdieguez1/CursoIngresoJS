...
function mostrar() {
	...
	while (contadorProductos < cantidadRequerida) {
		...
		switch (respuestaTipoProducto) {
			...
			default:
				//es alcohol
				if (precioProducto < precioAlcoholMasBarato || contadorComprasAlcohol == 0) {
	...
	if (contadorComprasAlcohol == 0) {
		alert("a) Fabricante de alcohol mas barato: " + fabricanteAlcoholMasBarato + ". Cantidad: " + cantidadAlcoholMasBarato);
	}
	...
	alert("c) Total de jabones comprados: " + totalJabonesComprados);
}